...
(function () {
  ...
  const calculateAssessResults = (testId, resp) => {
    ...
    // Shadow module (Dark Triad)
      ...
      if (Object.keys(subSums).length > 0) {
        results.subscales = {};
        for (const s in subSums) {
          results.subscales[s] = Math.round((subSums[s] / (subCounts[s] * 5)) * 100);
        }
      }
    } else if (testId === 'cognitive') {
      // v2: now uses keying for reverse items (Processing Style)
      results.score = calculateScaleScore(test.items, resp);
    } else if (testId === 'attachment') {
      let anxSum = 0, avSum = 0, secSum = 0, anxCount = 0, avCount = 0, secCount = 0;
      test.items.forEach((item, i) => {
        if (resp[i] && resp[i] !== null) {
          if (item.d === 'anx') { anxSum += resp[i]; anxCount++; }
          else if (item.d === 'sec') { secSum += resp[i]; secCount++; }
          else { avSum += resp[i]; avCount++; }
        }
      });
      results.anxiety = anxCount > 0 ? (anxSum / anxCount).toFixed(1) : '0';
      results.avoidance = avCount > 0 ? (avSum / avCount).toFixed(1) : '0';
      results.security = secCount > 0 ? (secSum / secCount).toFixed(1) : '0';
      const highAnx = results.anxiety > 3, highAv = results.avoidance > 3, highSec = results.security > 3;
      // If secure items score high and anxiety/avoidance low, clearly Secure
      results.style = highSec && !highAnx && !highAv ? 'Secure' : highAnx && !highAv ? 'Anxious' : !highAnx && highAv ? 'Avoidant' : highAnx && highAv ? 'Fearful' : 'Secure';
    } else if (testId === 'risk') {
    ...
    // Generic Likert tests — handles subcategory scores automatically
      ...
      if (test.subcategories) {
        ...
        results.subcategories = {};
        for (const s in subSums) {
          results.subcategories[s] = Math.round((subSums[s] / (subCounts[s] * 5)) * 100);
        }
      }
      // Clinical test severity bands
      if (test.clinical) {
        const rawSum = Object.values(resp).filter(v => v !== null).reduce((a, b) => a + b, 0);
        if (testId === 'phq9') {
          results.severity = rawSum <= 4 ? 'Minimal' : rawSum <= 9 ? 'Mild' : rawSum <= 14 ? 'Moderate' : rawSum <= 19 ? 'Moderately Severe' : 'Severe';
        } else if (testId === 'gad7') {
          results.severity = rawSum <= 4 ? 'Minimal' : rawSum <= 9 ? 'Mild' : rawSum <= 14 ? 'Moderate' : 'Severe';
        } else if (testId === 'aq10') {
          const indicators = Object.entries(resp).filter(([i, v]) => {
            if (v === null) return false;
            const item = test.items[parseInt(i)];
            return item.k === '+' ? v >= 4 : v <= 2;
          }).length;
          results.indicators = indicators;
          results.elevated = indicators >= 6;
        }
      }
      ...
      results.foundations = {};
      for (const s in subSums) {
        results.foundations[s] = Math.round((subSums[s] / (subCounts[s] * 5)) * 100);
      }
    } else if (testId === 'emotional-intelligence') {
      results.score = calculateScaleScore(test.items, resp);
      ...
      results.dimensions = {};
      for (const s in subSums) {
        results.dimensions[s] = Math.round((subSums[s] / (subCounts[s] * 5)) * 100);
      }
    } else if (testId === 'conflict-style') {
      // Profile across 5 conflict modes
      const subSums = {}, subCounts = {};
      test.items.forEach((item, i) => {
        if (resp[i] !== undefined && resp[i] !== null && item.sub) {
          let value = resp[i];
      ...
      results.styles = {};
      for (const s in subSums) {
        results.styles[s] = Math.round((subSums[s] / (subCounts[s] * 5)) * 100);
      }
      // Primary style = highest scoring
      const sorted = Object.entries(results.styles).sort((a, b) => b[1] - a[1]);
