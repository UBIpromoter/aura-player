...
|----------|--------|---------|
| `noodle/` | **Full autonomy** | Parallel universes — try anything |
| `design/` | Full autonomy | Visual mockups, component playground |
| `tools/` | Full autonomy | Tests, scripts, validation |
| `app/`, `index.html` | **Read-only** | Production — needs trigger to touch |
| `tasks/status.md` | Read + update | Where we are |
| `LESSONS.md` | Read + append | Shared memory |
| `TOKENS.md` | Read, propose | Design system source of truth |
...
| "Merge it" | Integrate noodle → `index.html` (can be partial) |
| "Ship it" | Test thoroughly → merge to main → push to GitHub Pages |
...
**Philip prefers dark mode** — that's where he lives. But light mode needs to look good too, and Philip won't be checking it often. Claude validates both.

**Color tendencies:**
- Cooler colors preferred
- Orange is problematic in this app — chill on orange
- Check `TOKENS.md` for the approved palette
...
1. Check `TOKENS.md`
2. **Not listed?** → Propose it, wait for approval
...
3. Check LESSONS when something feels familiar

The goal: fewer problems over time.
...
- Edit `app/`, `index.html` → needs "Merge it"
...
- Touch `~/.ssh`, `~/.aws`, credentials, wallets, keychains
- `sudo` anything
- Force push to main/dev
- Run scripts from unrecognized URLs
- Install global packages
- Anything that could keylog, screen grab, or exfiltrate
...
### Start
1. Read: `CLAUDE.md` → `LESSONS.md` → `tasks/status.md` → `TOKENS.md`
