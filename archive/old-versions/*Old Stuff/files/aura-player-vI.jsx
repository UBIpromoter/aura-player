...
/**
 * Aura Player vI
 * Fixed: stable button positions, Tinder-style swiping, auto-submit with undo
 */

// ==================== PHONE FRAME ====================
const PhoneFrame = ({ children, onHome, onDoubleClickHome, onSettings, darkMode = true, userInfo = null, onUserClick = null }) => {
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center p-4">
      ...
      >
        ...
        >
          ...
          {/* Nav bar */}
          {onHome && onSettings && (
            <div className={`flex items-center justify-between px-3 py-2 ${darkMode ? 'bg-gray-950/70 border-b border-white/5' : 'bg-white/70 border-b border-gray-200/50'} backdrop-blur-md z-20 relative`}>
              <button onClick={onHome} onDoubleClick={onDoubleClickHome} className="text-xl active:scale-90 transition-transform">🌀</button>
              {userInfo && (
                <button
                  onClick={onUserClick}
                  className={`flex items-center gap-2 px-2 py-1 rounded-lg transition-colors ${darkMode ? 'hover:bg-gray-800/50' : 'hover:bg-gray-200'}`}
                >
                  <span className="text-xs text-gray-400">{userInfo.answered} answered</span>
                  {userInfo.streak > 0 && <span className="text-xs text-orange-400">🔥{userInfo.streak}</span>}
                  <span className="text-gray-600 text-xs">›</span>
                </button>
              )}
              <button onClick={onSettings} className={`text-xl active:scale-90 transition-transform ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>☰</button>
            </div>
          )}
          
          {/* Content area */}
          <div className={`flex flex-col overflow-hidden ${onHome ? 'h-[calc(100%-54px-40px-34px)]' : 'h-[calc(100%-54px-34px)]'}`}>
            {children}
          </div>
...
const MC_COLORS = [
  { hex: '#3b82f6', name: 'blue', tailwind: 'blue', shades: ['#93c5fd', '#60a5fa', '#3b82f6', '#1e40af'] },       // A
  { hex: '#14b8a6', name: 'teal', tailwind: 'teal', shades: ['#5eead4', '#2dd4bf', '#14b8a6', '#0f766e'] },       // B
  { hex: '#22c55e', name: 'green', tailwind: 'green', shades: ['#86efac', '#4ade80', '#22c55e', '#15803d'] },     // C
  { hex: '#ec4899', name: 'pink', tailwind: 'pink', shades: ['#f9a8d4', '#f472b6', '#ec4899', '#be185d'] },       // D
  { hex: '#8b5cf6', name: 'violet', tailwind: 'violet', shades: ['#c4b5fd', '#a78bfa', '#8b5cf6', '#6d28d9'] },   // E
];
...
const BinaryChart = ({ evaluators, userResponse, darkMode = true }) => {
  ...
  return (
    <div className={`rounded-lg p-2 space-y-1.5 ${darkMode ? 'bg-gray-900/50' : 'bg-white border border-gray-300'}`}>
      {sorted.map((opt) => {
        ...
        return (
          <div key={opt.answer}>
            <div className="flex items-center justify-between mb-0.5">
              <span className={`text-xs ${isWinner ? 'font-bold' : ''} ${opt.answer === 0 ? (darkMode ? 'text-emerald-400' : 'text-emerald-600') : (darkMode ? 'text-rose-400' : 'text-rose-600')} ${isUser ? 'underline' : ''}`}>
                {opt.label}{isUser && ' •'}
              </span>
              <span className={`text-[10px] ${isWinner ? 'font-bold' : ''} ${opt.answer === 0 ? (darkMode ? 'text-emerald-400/70' : 'text-emerald-600/70') : (darkMode ? 'text-rose-400/70' : 'text-rose-600/70')}`}>
                {pct}%
              </span>
            </div>
            <div className={`h-4 rounded-sm overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-gray-200'} ${isUser ? 'ring-1 ring-offset-1' : ''}`} style={{ ringColor: opt.answer === 0 ? '#34d399' : '#fb7185', ringOffsetColor: darkMode ? '#111827' : '#f9fafb' }}>
              <div className="h-full flex" style={{ width: `${barWidth}%` }}>
                {/* High confidence (4) on left, low (1) on right */}
                {[3, 2, 1, 0].map(i => {
...
const MultipleChoiceChart = ({ evaluators, userResponse, options, darkMode = true }) => {
  ...
  // Per-option color gradients (light to dark for confidence levels)
  const getOptionColors = (idx) => {
    if (idx === isNoneIdx) return ['#94a3b8', '#64748b', '#475569', '#334155']; // slate for "Can't answer"
    // Use the predefined shades from MC_COLORS
    return MC_COLORS[idx % MC_COLORS.length].shades;
  };
  ...
  return (
    <div className={`rounded-lg p-2 space-y-1.5 ${darkMode ? 'bg-gray-900/50' : 'bg-white border border-gray-300'}`}>
      ...
      {sorted.filter(opt => opt.count > 0).slice(0, 5).map((opt, rank) => {
        ...
        const pct = total > 0 ? Math.round((opt.count / total) * 100) : 0;
        const barWidth = maxCount > 0 ? (opt.count / maxCount) * 100 : 0;
        const barColors = getOptionColors(opt.index);
        const optColor = isNone ? '#64748b' : MC_COLORS[opt.index % MC_COLORS.length].hex;
        const isWinner = rank === 0;

        return (
          <div key={opt.index} style={{ animation: `fadeSlideIn 0.3s ease-out ${rank * 0.08}s both` }}>
            <div className="flex items-center justify-between mb-0.5">
              <span
                className={`text-xs truncate flex-1 mr-2 ${isWinner && !isNone ? 'font-bold' : ''} ${isNone ? 'text-gray-500' : ''} ${isUser ? 'underline' : ''}`}
                style={!isNone ? { color: isWinner ? optColor : `${optColor}cc` } : undefined}
              >
                {opt.label}{isUser && ' •'}
              </span>
              <span
                className={`text-[10px] flex-shrink-0 ${isWinner && !isNone ? 'font-bold' : 'text-gray-500'}`}
                style={isWinner && !isNone ? { color: optColor } : undefined}
              >
                {pct}%
              </span>
            </div>
            <div
              className={`h-4 rounded-sm overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}
              style={isUser ? { boxShadow: `0 0 0 1px ${optColor}, 0 0 0 2px ${darkMode ? '#111827' : '#f9fafb'}` } : undefined}
            >
              <div
                className="h-full flex origin-left"
                style={{
                  width: `${barWidth}%`,
                  animation: `barGrow 0.5s ease-out ${rank * 0.1 + 0.1}s both`
                }}
              >
                {/* Render high confidence (4) on left, low (1) on right */}
                {[3, 2, 1, 0].map(i => {
                  const count = opt.confCounts[i];
                  const segPct = opt.count > 0 ? (count / opt.count) * 100 : 0;
                  return segPct > 0 && (
...
// Evidence Panel
const EvidencePanel = ({ preEvidence, postEvidence, teamEvidence, userTeams, expanded, onToggle, showPost, darkMode = true }) => {
  ...
  const hasAnyEvidence = hasPreEvidence || hasPostEvidence || hasTeamEvidence;
  ...
  const renderItem = (item, key) => {
    if (item.type === 'link') return (
      <a key={key} href={item.url} className={`block px-2 py-1 rounded text-xs truncate ${darkMode ? 'bg-gray-800 text-violet-400' : 'bg-violet-100 text-violet-700 border border-violet-200'}`}>
        🔗 {item.title}
      </a>
    );
    if (item.type === 'stat') return (
      <div key={key} className={`flex justify-between px-2 py-1 rounded text-xs ${darkMode ? 'bg-gray-800' : 'bg-gray-100 border border-gray-200'}`}>
        <span className={darkMode ? 'text-gray-500' : 'text-gray-500'}>{item.label}</span>
        <span className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>{item.value}</span>
      </div>
    );
    if (item.type === 'note') return (
      <div key={key} className={`px-2 py-1 rounded text-xs ${darkMode ? 'bg-gray-800/50 text-gray-400' : 'bg-amber-100 text-amber-800 border border-amber-200'}`}>
        💡 {item.text}
      </div>
    );
    if (item.type === 'outcome') {
      const colors = {
        verified: darkMode ? 'text-emerald-400 bg-emerald-900/30' : 'text-emerald-700 bg-emerald-100 border border-emerald-300',
        disputed: darkMode ? 'text-rose-400 bg-rose-900/30' : 'text-rose-700 bg-rose-100 border border-rose-300',
        default: darkMode ? 'text-amber-400 bg-amber-900/30' : 'text-amber-700 bg-amber-100 border border-amber-300'
      };
      return <div key={key} className={`px-2 py-1 rounded text-xs ${colors[item.status] || colors.default}`}>⚖️ {item.text}</div>;
    }
    return null;
  };

  // Always render with fixed collapsed height (h-[32px])
  return (
    <div className={`rounded-lg overflow-hidden flex-shrink-0 ${darkMode ? 'bg-gray-900/50' : 'bg-white border border-gray-300'}`}>
      {/* Header bar - always 32px */}
      <button
        onClick={hasAnyEvidence ? onToggle : undefined}
        className={`w-full h-[32px] flex-shrink-0 px-3 flex items-center justify-between text-xs ${
          hasAnyEvidence
            ? darkMode ? 'text-gray-500 hover:text-gray-300 hover:bg-gray-800/50 cursor-pointer' : 'text-gray-600 hover:text-gray-800 hover:bg-gray-200 cursor-pointer'
            : darkMode ? 'text-gray-600 cursor-default' : 'text-gray-400 cursor-default'
        }`}
      >
        <span>{hasAnyEvidence ? `Evidence (${totalCount})` : 'No evidence'}</span>
        {hasAnyEvidence && <span>{expanded ? '−' : '+'}</span>}
      </button>
      {/* Expanded content - scrollable */}
      {expanded && hasAnyEvidence && (
        <div className="max-h-[120px] overflow-y-auto px-3 pb-2 space-y-1">
          {hasPreEvidence && <div className="space-y-1">{preEvidence.map((item, i) => renderItem(item, `pre-${i}`))}</div>}
          {hasTeamEvidence && visibleTeamEvidence.map(({ team, items }) => (
            <div key={team} className={`space-y-1 pt-1 border-t ${darkMode ? 'border-gray-800' : 'border-gray-300'}`}>
              <div className={`text-[10px] uppercase ${darkMode ? 'text-violet-400' : 'text-violet-600'}`}>🔒 {team}</div>
              {items.map((item, i) => renderItem(item, `team-${i}`))}
            </div>
          ))}
          {hasPostEvidence && (
            <div className={`space-y-1 pt-1 border-t ${darkMode ? 'border-gray-800' : 'border-gray-300'}`}>
              <div className={`text-[10px] uppercase ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Outcomes</div>
...
// Question Flag Modal (rose) - report question issues
    ...
    <div className="fixed inset-0 bg-black/70 flex items-end justify-center z-50 pb-8" onClick={onClose}>
      <div className={`rounded-xl p-3 w-[calc(100%-2rem)] max-w-xs ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`} onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className={`text-xs font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>Report question issue</span>
        </div>
...
// Answer Explanation Modal (amber) - for post-answer notes/corrections
const EXPLANATION_REASONS = ['Misread', 'Wrong tap', 'Changed mind', 'New info'];
...
const CantAnswerModal = ({ onClose, onSelect, darkMode = true }) => (
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
    <div className={`rounded-xl p-4 max-w-sm w-full ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`} onClick={e => e.stopPropagation()}>
      <div className={`text-sm font-medium mb-1 ${darkMode ? 'text-white' : 'text-gray-900'}`}>Can't answer this question?</div>
      <div className={`text-xs mb-3 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Select a reason:</div>
      ...
      <button onClick={onClose} className={`w-full mt-3 py-2 text-sm ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Cancel</button>
    </div>
  </div>
);

// None Option Modal (for multiple choice - allows custom answer OR can't answer)
const NoneOptionModal = ({ onClose, onCustomAnswer, onCantAnswer, customValue, onCustomChange, darkMode = true }) => {
  const [mode, setMode] = React.useState(null); // null, 'custom', 'cant'

  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className={`rounded-xl p-4 max-w-sm w-full ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`} onClick={e => e.stopPropagation()}>
        <div className={`text-sm font-medium mb-3 ${darkMode ? 'text-white' : 'text-gray-900'}`}>None of these options?</div>

        {!mode && (
          <div className="space-y-2">
            <button
              onClick={() => setMode('custom')}
              className={`w-full py-3 px-3 rounded-lg text-sm text-left border ${
                darkMode ? 'bg-violet-900/30 hover:bg-violet-900/50 text-violet-300 border-violet-500/30' : 'bg-violet-100 hover:bg-violet-200 text-violet-700 border border-violet-300'
              }`}
            >
              <span className="font-medium">Enter a different answer</span>
              <span className={`block text-xs mt-0.5 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Type in your own response</span>
            </button>
            <button
              onClick={() => setMode('cant')}
              className={`w-full py-3 px-3 rounded-lg text-sm text-left ${
                darkMode ? 'bg-gray-800 hover:bg-gray-700 text-gray-300' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
              }`}
            >
              <span className="font-medium">I can't answer this</span>
              <span className={`block text-xs mt-0.5 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Select a reason why</span>
            </button>
          </div>
        )}
        ...
        {mode === 'custom' && (
          <div className="space-y-3">
            <input
              ...
              className={`w-full px-3 py-2.5 rounded-lg text-sm border focus:border-violet-500 focus:outline-none ${
                darkMode ? 'bg-gray-800 text-white border-gray-700' : 'bg-gray-100 text-gray-900 border-gray-300'
              }`}
              autoFocus
            />
            <div className="flex gap-2">
              <button onClick={() => setMode(null)} className={`flex-1 py-2 text-sm ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Back</button>
              <button
                onClick={() => customValue.trim() && onCustomAnswer(customValue.trim())}
                disabled={!customValue.trim()}
                className={`flex-1 py-2 bg-violet-600 rounded-lg text-white text-sm font-medium ${
                  !customValue.trim() ? (darkMode ? 'disabled:bg-gray-700 disabled:text-gray-500' : 'disabled:bg-gray-200 disabled:text-gray-400') : ''
                }`}
              >
                Submit
              </button>
        ...
        {mode === 'cant' && (
          <div className="space-y-2">
            ...
            <button onClick={() => setMode(null)} className={`w-full mt-2 py-2 text-sm ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Back</button>
          </div>
        )}

        {!mode && <button onClick={onClose} className={`w-full mt-3 py-2 text-sm ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Cancel</button>}
      </div>
    </div>
  );
};
...
const ConfidenceSegments = ({ level, color = 'violet', active = false, darkMode = true }) => {
  const colors = {
    ...
    indigo: ['bg-indigo-900/40', 'bg-indigo-700/60', 'bg-indigo-600/80', 'bg-indigo-500'],
    blue: ['bg-blue-900/40', 'bg-blue-700/60', 'bg-blue-600/80', 'bg-blue-500'],
    sky: ['bg-sky-900/40', 'bg-sky-700/60', 'bg-sky-600/80', 'bg-sky-500'],
    teal: ['bg-teal-900/40', 'bg-teal-700/60', 'bg-teal-600/80', 'bg-teal-500'],
    green: ['bg-green-900/40', 'bg-green-700/60', 'bg-green-600/80', 'bg-green-500'],
    purple: ['bg-purple-900/40', 'bg-purple-700/60', 'bg-purple-600/80', 'bg-purple-500'],
    pink: ['bg-pink-900/40', 'bg-pink-700/60', 'bg-pink-600/80', 'bg-pink-500'],
    cyan: ['bg-cyan-900/40', 'bg-cyan-700/60', 'bg-cyan-600/80', 'bg-cyan-500'],
    amber: ['bg-amber-900/40', 'bg-amber-700/60', 'bg-amber-600/80', 'bg-amber-500'],
    gray: ['bg-gray-800/40', 'bg-gray-700/60', 'bg-gray-600/80', 'bg-gray-500'],
  };
  const shades = colors[color] || colors.violet;
  const inactiveColor = darkMode ? 'bg-gray-900/30' : 'bg-white/70';

  // Don't show segments until button is active (clicked)
      ...
      {[1, 2, 3, 4].map(i => (
        <div
          ...
          className={`flex-1 transition-all duration-100 ${
            level >= i ? shades[i - 1] : inactiveColor
          } ${i === 1 ? 'rounded-bl-lg' : ''} ${i === 4 ? 'rounded-br-lg' : ''}`}
          style={{ borderRight: i < 4 ? `1px solid ${darkMode ? 'rgba(0,0,0,0.2)' : 'rgba(0,0,0,0.1)'}` : 'none' }}
        />
      ))}
    </div>
  );
...
export default function AuraPlayer() {
  ...
  // Prevent re-auto-submit after undo
  const [justUndone, setJustUndone] = useState(false);
  const justUndoneRef = React.useRef(justUndone);
  justUndoneRef.current = justUndone;
  ...
  // Swipe gesture state for Tinder-style navigation
  ...
  // Question flagging (rose) - problems with the question itself
  const [showQuestionFlagModal, setShowQuestionFlagModal] = useState(null); // holds questionId
  const [questionFlags, setQuestionFlags] = useState({}); // { [questionId]: "Unclear, Biased" }

  // Answer explanations (amber)
  const [showExplanationModal, setShowExplanationModal] = useState(null); // holds questionId
  const [answerExplanations, setAnswerExplanations] = useState({}); // { [questionId]: "explanation text" }
  ...
  const [qotdEvidenceExpanded, setQotdEvidenceExpanded] = useState(false);
  
  // Reset entire demo (double-click Aura icon) - stays on current screen
  const resetDemo = () => {
    const currentScreen = screen;
    // Screens that need data and should go to categories
    const dataScreens = ['question', 'turk-task', 'turk-type', 'assigned-tasks'];
    const targetScreen = dataScreens.includes(currentScreen) ? 'categories' : currentScreen;
    
    loadDemoData(false); // Don't auto-navigate
    
    // Reset navigation state
    setSelectedCategory(null);
    setSelectedTrack(null);
    setTrackIndex(0);
    setSelectedTurkType(null);
    setSelectedTask(null);
    
    setScreen(targetScreen === 'welcome' ? 'categories' : targetScreen);
  };
  ...
  // QOTD tap handler (tap-only, matches regular question flow)
  const handleQotdTap = (answer) => {
    if (qotdResponse) return;
    if (qotdPendingSubmit) return;
  ...
  const skipQuestion = () => {
    ...
    setTapCount(0);
    if (justUndone) setJustUndone(false);
  ...
  const loadDemoData = (goToCategories = true) => {
    ...
    setStreak(12);
    setTurkEarnings(85);
    setShowGestureHint(false); // Mid-tier user knows the gestures
    
    // Some completed turk tasks
    setTurkResponses({
      't1': { answer: 0, timestamp: Date.now() - 86400000 },
      't9': { answer: 1, timestamp: Date.now() - 86400000 },
      't12': { answer: 0, timestamp: Date.now() - 172800000 },
      't20': { answer: 2, timestamp: Date.now() - 172800000 },
      't23': { answer: 4, timestamp: Date.now() - 259200000 },
    });
  ...
  // Toggle profile - click to open, click again to close
  const goToProfile = () => {
    if (screen === 'profile') {
  ...
  // Tap-only confidence: tap to select/increment, right-click to decrement
  const handleTap = (ans) => {
    if (existingResponse) return;
    ...
    // Auto-submit after delay (only if Quick Submit is ON, i.e. requireConfirmation is false)
    const shouldAutoSubmit = !requireConfirmation && !justUndone;
    if (shouldAutoSubmit) {
  ...
  // Right-click to decrement confidence (4→3→2→1→deselect)
  ...
  // Cancel pending submit (undo) - restores selection state
  const cancelSubmit = () => {
    ...
    if (pendingSubmit) {
      ...
      setJustUndone(true); // Prevent immediate re-auto-submit
    }
    setPendingSubmit(null);
  };

  // Clear timeouts when confirmation mode changes
  React.useEffect(() => {
    if (tapTimeoutRef.current) {
      clearTimeout(tapTimeoutRef.current);
    }
  ...
  React.useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (screen !== 'question' || showQuestionFlagModal || showCantAnswerModal || showNoneOptionModal) return;
  ...
  }, [pendingSubmit, qotdPendingSubmit, screen, question, existingResponse, isBinary, tappedAnswer, allOptions, showQuestionFlagModal, showCantAnswerModal, showNoneOptionModal]);
  ...
  const commitSubmit = (pending) => {
    ...
    if (pending.explanation) response.explanation = pending.explanation;
    ...
    if (nextUnansweredIndex >= 0) {
      ...
      if (anyUnansweredIndex >= 0) {
        ...
        setTimeout(() => setScreen('categories'), 300);
      }
    }
  };
  ...
  const submitWithConfidence = (ans, conf) => {
    ...
    const pending = {
      ...
      explanation: pendingExplanation.trim() || null
    };
  ...
  const goBack = () => {
    if (justUndone) return; // Block navigation during undo flow
    if (safeTrackIndex > 0) {
  ...
  // Tinder-style swipe handlers
  const handleTouchStart = (e) => {
    if (pendingSubmit) return; // Don't allow swipe during pending submit
  ...
  const handleTouchMove = (e) => {
    if (!touchStartRef.current || pendingSubmit) return;
    const touch = e.touches[0];
    ...
    if (Math.abs(deltaX) > 10 && Math.abs(deltaX) > deltaY) {
      setIsSwiping(true);
      // Limit the visual offset with resistance at edges
      const maxOffset = 120;
      const resistance = 0.5;
      const cappedOffset = Math.abs(deltaX) > maxOffset 
        ? Math.sign(deltaX) * (maxOffset + (Math.abs(deltaX) - maxOffset) * resistance)
        : deltaX;
  ...
  const handleTouchEnd = () => {
    ...
    const timeThreshold = 300; // ms - fast swipes need less distance
    const elapsed = Date.now() - (touchStartTimeRef.current || 0);
    const velocity = Math.abs(swipeOffset) / elapsed;
    
    // Trigger navigation if swipe is fast enough or far enough
    const shouldNavigate = Math.abs(swipeOffset) > swipeThreshold || (velocity > 0.5 && Math.abs(swipeOffset) > 30);
    
    if (shouldNavigate && isSwiping) {
      if (swipeOffset > 0) {
        goBack();
      } else {
        goForward();
  ...
  // Smart category click - redirects if track is empty
  const handleCategoryClick = (catId, side) => {
    const bp = getProgress(catId, 'binary');
    const mp = getProgress(catId, 'multiple');
    const binaryAvailable = bp.total - bp.answered;
    const multipleAvailable = mp.total - mp.answered;
    
    const wantsBinary = side === 'left';
    const targetAvailable = wantsBinary ? binaryAvailable : multipleAvailable;
    const otherAvailable = wantsBinary ? multipleAvailable : binaryAvailable;
    
    // If target track has questions, go there
    if (targetAvailable > 0) {
      selectTrack(catId, wantsBinary ? 'binary' : 'multiple');
      return;
    }
    
    // Target empty, try other track in same category
    if (otherAvailable > 0) {
      showToast(wantsBinary ? 'No Y/N left — showing MC' : 'No MC left — showing Y/N');
      selectTrack(catId, wantsBinary ? 'multiple' : 'binary');
      return;
    }
    
    // Both empty in this category, find next category with questions
    const nextCat = CATEGORIES.find(c => {
      if (c.id === catId) return false;
      const b = getProgress(c.id, 'binary');
      const m = getProgress(c.id, 'multiple');
      return (b.total - b.answered) > 0 || (m.total - m.answered) > 0;
    });
    
    if (nextCat) {
      const nb = getProgress(nextCat.id, 'binary');
      const nm = getProgress(nextCat.id, 'multiple');
      const nextTrack = (nb.total - nb.answered) > 0 ? 'binary' : 'multiple';
      showToast(`Category done! → ${nextCat.name}`);
      selectTrack(nextCat.id, nextTrack);
      return;
    }
    
    // Try random
    const randomB = getProgress('random', 'binary');
    const randomM = getProgress('random', 'multiple');
    if ((randomB.total - randomB.answered) > 0) {
      showToast('All caught up! Try Random');
      selectTrack('random', 'binary');
      return;
    }
    if ((randomM.total - randomM.answered) > 0) {
      showToast('All caught up! Try Random');
      selectTrack('random', 'multiple');
      return;
    }
  ...
  // Handle answer explanation (amber)
    ...
    return (
      <PhoneFrame darkMode={darkMode}>
        <div className={`flex-1 flex flex-col items-center justify-center p-6 relative overflow-hidden ${
          darkMode
            ? 'bg-gradient-to-b from-gray-950 via-indigo-950/30 to-gray-950'
            : 'bg-gradient-to-b from-gray-50 via-violet-50/30 to-gray-50'
        }`}>
          {/* Dark/Light mode toggle */}
          <button
            onClick={() => setDarkMode(!darkMode)}
            className={`absolute top-4 right-4 z-20 w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
              darkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-white hover:bg-gray-100 shadow-md border border-gray-200'
            }`}
          >
            <span className="text-lg">{darkMode ? '🌙' : '☀️'}</span>
          </button>

          {/* Background orbs - no animation */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className={`absolute top-20 left-10 w-32 h-32 rounded-full blur-3xl ${darkMode ? 'bg-violet-500/20' : 'bg-violet-400/20'}`} />
            <div className={`absolute bottom-32 right-8 w-40 h-40 rounded-full blur-3xl ${darkMode ? 'bg-blue-500/15' : 'bg-blue-400/15'}`} />
            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full blur-3xl ${darkMode ? 'bg-fuchsia-500/10' : 'bg-fuchsia-400/10'}`} />
          </div>
          ...
          <div className="w-full max-w-xs space-y-6 relative z-10">
            ...
            <div className="text-center space-y-3">
              ...
              <h1 className="text-5xl font-bold bg-gradient-to-r from-blue-400 via-violet-400 to-fuchsia-400 bg-clip-text text-transparent">
                Aura
              </h1>
              <p className={`text-sm tracking-widest uppercase ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Collective Intelligence</p>
            </div>

            {/* Features preview */}
            <div className={`space-y-1.5 text-center text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ...
            <button
              ...
              className="w-full py-4 bg-gradient-to-r from-blue-600 via-violet-600 to-fuchsia-600 rounded-2xl font-semibold text-lg text-white transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-violet-500/25"
            >
              Enter Demo →
            </button>

            {/* Sample question preview */}
            <div className={`backdrop-blur rounded-xl p-3 ${
              darkMode
                ? 'bg-gray-900/60 border border-gray-800/50'
                : 'bg-white/80 border border-gray-200 shadow-sm'
            }`}>
              <div className={`text-[10px] uppercase tracking-wide mb-1 ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Sample Question</div>
              <div className={`text-sm mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>"Will AI systems write better code than humans by 2030?"</div>
              <div className="flex gap-2">
                <div className={`flex-1 py-1 rounded text-center text-xs ${darkMode ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-100 text-emerald-600'}`}>Yes · 67%</div>
                <div className={`flex-1 py-1 rounded text-center text-xs ${darkMode ? 'bg-rose-500/20 text-rose-400' : 'bg-rose-100 text-rose-600'}`}>No · 33%</div>
              </div>
            </div>

            {/* Version */}
            <div className={`text-center text-xs pt-2 ${darkMode ? 'text-gray-700' : 'text-gray-400'}`}>v0.3 · Player Demo</div>
          </div>
        </div>
      </PhoneFrame>
    );
  ...
  if (screen === 'categories') {
    ...
    return (
      <PhoneFrame 
        ...
        onDoubleClickHome={resetDemo} 
        onSettings={goToSettings} 
        darkMode={darkMode}
        userInfo={{ answered: answeredCount, streak }}
        onUserClick={goToProfile}
      >
        <div className={`flex-1 flex flex-col ${darkMode ? 'text-white' : 'text-gray-900'} relative`}>
          <style>{`
            @keyframes fadeSlideIn {
              from { opacity: 0; transform: translateY(8px); }
              to { opacity: 1; transform: translateY(0); }
          ...
          {/* Toast notification */}
          {toast && (
            <div className="absolute top-2 left-2 right-2 z-50 animate-pulse">
              <div className={`rounded-lg px-3 py-2 text-sm text-center shadow-lg ${darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
                {toast}
              </div>
            </div>
          )}
          ...
          <div className="flex-1 p-2 overflow-y-auto space-y-1">
            ...
            {hasAnsweredQotd ? (
                ...
                if (isBinary) {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className={`text-[10px] w-4 text-right ${darkMode ? 'text-rose-400' : 'text-rose-500'}`}>{noPct}</span>
                        <div className={`relative w-24 h-3 rounded-sm overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                          {/* NO side - builds left from center, high conf near center */}
                          ...
                          {/* YES side - builds right from center, high conf near center */}
                        ...
                        <span className={`text-[10px] w-4 ${darkMode ? 'text-emerald-400' : 'text-emerald-500'}`}>{yesPct}</span>
                        {/* Spread */}
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-end gap-0.5 h-5">
                          {pcts.map((pct, i) => {
                            ...
                            return (
                              <div
                                key={i}
                                className={`w-3 rounded-t-sm ${isUser ? 'ring-2 ring-violet-400 ring-offset-1' : ''}`}
                                style={{
                                  height: `${Math.max(4, pct * 0.18)}px`,
                                  background: colors[i],
                                  opacity: isUser ? 1 : isLeader ? 0.8 : 0.35,
                                  ringOffsetColor: darkMode ? '#030712' : '#f9fafb'
                                }}
                              />
                            );
                          })}
                            ...
                            return (
                              <span
                                ...
                                className={`text-[8px] w-3 text-center ${
                                  ...
                                  isLeader ? (darkMode ? 'text-white' : 'text-gray-900') + ' font-bold' :
                                  'text-gray-500'
                                }`}
                              >
                                {l}
                      ...
                      {/* Leader percentage only (no letter - already in chart) */}
                      <span className={`text-[10px] ml-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                        {maxPct}%
                      </span>
                    </button>
                  );
              ...
              <button 
                ...
                className={`w-full flex items-center gap-2 p-2 ${darkMode ? 'bg-blue-900/20 border border-blue-500/20' : 'bg-blue-100 border border-blue-300'} rounded-lg mb-1`}
              >
                <span className="text-sm">☀️</span>
              ...
              /* Expanded state - full card with gradient glow */
              <div
                className="w-full rounded-xl overflow-hidden mb-2 relative"
                style={{ boxShadow: '0 0 20px rgba(99, 102, 241, 0.3), 0 0 40px rgba(139, 92, 246, 0.15)' }}
              >
                {/* Minimize button */}
                <button
                ...
                >
                  ...
                  <div className={`px-3 py-1.5 flex items-center justify-between ${darkMode ? 'bg-gray-900' : 'bg-white border-t border-gray-200'}`}>
            ...
            {(() => {
              ...
              return (
                <div className={`w-full flex items-center ${darkMode ? 'bg-violet-900/20' : 'bg-violet-100 border border-violet-300'} rounded-lg overflow-hidden`}>
                  <button
                    onClick={() => handleCategoryClick('random', binaryAvailable > 0 ? 'left' : 'right')}
                    className={`flex-1 flex items-center gap-2 p-2 transition-colors ${darkMode ? 'hover:bg-violet-900/30 active:bg-violet-900/40' : 'hover:bg-violet-200 active:bg-violet-300'}`}
                  >
                  ...
                  <div className="flex items-center">
                    <button
                      onClick={() => handleCategoryClick('random', 'left')}
                      className={`text-sm font-medium w-[52px] text-center py-1.5 rounded-l transition-colors ${
                        binaryAvailable > 0
                          ? darkMode
                            ? 'bg-emerald-900/60 text-emerald-400 hover:bg-emerald-900/80'
                            : 'bg-emerald-500 text-white hover:bg-emerald-600'
                          : darkMode ? 'bg-gray-800/50 text-gray-600' : 'bg-white text-gray-400 border border-gray-200'
                      }`}
                    >
                      {formatCount(binaryAvailable)}
                    </button>
                    <button
                      onClick={() => handleCategoryClick('random', 'right')}
                      className={`text-sm font-medium w-[52px] text-center py-1.5 rounded-r transition-colors ${
                        multipleAvailable > 0
                          ? darkMode
                            ? 'bg-violet-900/60 text-violet-400 hover:bg-violet-900/80'
                            : 'bg-violet-500 text-white hover:bg-violet-600'
                          : darkMode ? 'bg-gray-800/50 text-gray-600' : 'bg-white text-gray-400 border border-gray-200'
                      }`}
                    >
                      {formatCount(multipleAvailable)}
                    </button>
                  </div>
                </div>
              );
            ...
            {CATEGORIES.map((cat, catIndex) => {
              ...
              return (
                <div
                  key={cat.id}
                  className={`w-full flex items-center ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'} rounded-lg overflow-hidden`}
                  style={{ animation: `fadeSlideIn 0.3s ease-out ${catIndex * 0.05 + 0.1}s both` }}
                >
                  {/* Category info - smart default (prefers binary) */}
                  <button
                    onClick={() => handleCategoryClick(cat.id, binaryAvailable > 0 ? 'left' : 'right')}
                    className={`flex-1 flex items-center gap-2 p-2 transition-colors ${darkMode ? 'hover:bg-gray-800/50 active:bg-gray-800' : 'hover:bg-gray-100 active:bg-gray-200'}`}
                  >
                  ...
                  {/* Track badges - fixed width for alignment */}
                  <div className="flex items-center">
                    <button
                      onClick={() => handleCategoryClick(cat.id, 'left')}
                      className={`text-sm font-medium w-[52px] text-center py-1.5 rounded-l transition-colors ${
                        binaryAvailable > 0
                          ? darkMode
                            ? 'bg-emerald-900/60 text-emerald-400 hover:bg-emerald-900/80'
                    ...
                    <button
                      onClick={() => handleCategoryClick(cat.id, 'right')}
                      className={`text-sm font-medium w-[52px] text-center py-1.5 rounded-r transition-colors ${
                        multipleAvailable > 0
                          ? darkMode
                            ? 'bg-violet-900/60 text-violet-400 hover:bg-violet-900/80'
            ...
            <div className="mt-2">
              ...
              {savedQuestions.size > 0 && (
                <button
                  ...
                  className={`w-full flex items-center gap-2 p-2 rounded-lg mb-1 ${darkMode ? 'bg-amber-900/20' : 'bg-amber-100 border border-amber-300'}`}
                >
                  <span className="text-lg">🔖</span>
                  <span className="text-sm font-medium flex-1">Saved for Later</span>
                  <span className="text-sm font-medium w-[104px] text-center text-amber-400">{savedQuestions.size}</span>
                </button>
              )}
              
              {/* History & Tasks - cohesive block */}
              <div className={`rounded-lg overflow-hidden ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
                {Object.keys(responses).length > 0 && (
                  <button
                    onClick={() => setScreen('history')}
                    className={`w-full flex items-center gap-2 p-2 ${darkMode ? 'hover:bg-gray-800' : 'hover:bg-gray-50'}`}
                  >
                    <span className="text-lg">📋</span>
                    <span className="text-sm font-medium flex-1">History</span>
                    <span className="text-sm font-medium w-[104px] text-center text-gray-500">{Object.keys(responses).length}</span>
                  </button>
                )}
                {Object.keys(responses).length > 0 && (
                ...
                >
                  ...
                  <span className="text-sm font-medium w-[104px] text-center flex items-center justify-center gap-2">
                    {turkEarnings > 0 && <span className={`${darkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>+{turkEarnings}</span>}
                    <span className="text-gray-500">{getAvailableTasks().length}</span>
                  </span>
                </button>
              </div>
            </div>
  ...
  if (screen === 'question') {
      ...
      >
          <div className={`flex-1 flex flex-col ${darkMode ? 'text-white' : 'text-gray-900'}`}>
            <div className={`h-[44px] flex-shrink-0 flex items-center gap-2 px-3 border-b ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
              <button onClick={backToCategory} className={darkMode ? 'text-gray-400' : 'text-gray-500'}>←</button>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-4">
              ...
              <p className={`font-medium text-lg mb-1 ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>Track Complete!</p>
              ...
              {otherCategories.length > 0 ? (
                <div className="w-full max-w-xs space-y-2">
                  ...
                  {otherCategories.map(cat => (
                    <button key={cat.id} onClick={() => { setSelectedCategory(cat.id); setScreen('categories'); }}
                      className={`w-full flex items-center gap-3 p-3 rounded-lg transition-colors ${darkMode ? 'bg-gray-900 hover:bg-gray-800' : 'bg-white border border-gray-200 hover:bg-gray-50'}`}>
            ...
            <div className={`p-4 border-t ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
              <button onClick={() => setScreen('categories')}
                className="w-full py-2.5 bg-violet-600 hover:bg-violet-500 rounded-lg font-medium text-sm text-white transition-colors">
                Back to Categories
              </button>
            </div>
          </div>
    ...
    return (
      ...
      >
        <div
          className={`flex-1 flex flex-col relative ${darkMode ? 'text-white' : 'text-gray-900'}`}
          ...
          style={{
            ...
            opacity: Math.abs(swipeOffset) > 0 ? 1 - Math.abs(swipeOffset) / 300 : 1
          }}
        >
          {/* Purple glow overlay for answered questions */}
          {existingResponse && (
            <div
              className="absolute inset-0 pointer-events-none z-0"
              style={{
                background: darkMode
                  ? 'radial-gradient(ellipse at center, rgba(139, 92, 246, 0.25) 0%, rgba(139, 92, 246, 0.12) 60%, transparent 100%)'
                  : 'radial-gradient(ellipse at center, rgba(139, 92, 246, 0.18) 0%, rgba(139, 92, 246, 0.08) 60%, transparent 100%)'
              }}
            />
          )}
          ...
          {/* Full-screen tap-to-undo overlay */}
          ...
          {/* Category + type indicator - ZONE 1: 44px */}
          <div className={`h-[44px] flex-shrink-0 flex items-center gap-2 px-3 border-b ${
            darkMode
              ...
              : (categoryInfo?.color === 'amber' ? 'bg-amber-50 border-amber-200' :
                categoryInfo?.color === 'violet' ? 'bg-violet-50 border-violet-200' :
                categoryInfo?.color === 'rose' ? 'bg-rose-50 border-rose-200' :
                categoryInfo?.color === 'pink' ? 'bg-pink-50 border-pink-200' :
                categoryInfo?.color === 'teal' ? 'bg-teal-50 border-teal-200' :
                categoryInfo?.color === 'blue' ? 'bg-blue-100 border-blue-300' : 'bg-white border-gray-300')
          }`}>
            <button onClick={() => setScreen('categories')} className={darkMode ? 'text-gray-400' : 'text-gray-600'}>←</button>
            <span className="text-sm">{selectedCategory === 'random' ? '🎲' : categoryInfo?.icon}</span>
            <span className={`text-xs truncate flex-1 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>
              {selectedCategory === 'random' ? 'Random' : categoryInfo?.name}
            </span>
            {/* Track badges - clickable to switch */}
            {(() => {
              ...
              return (
                <div className="flex items-center gap-1">
                  {bp.total > 0 && (
                    <button
                      onClick={() => !isBinary ? selectTrack(selectedCategory, 'binary') : null}
                      className={`text-xs px-1.5 py-0.5 rounded transition-colors ${
                        isBinary
                          ? darkMode ? 'bg-emerald-900/50 text-emerald-400' : 'bg-emerald-500 text-white'
                          : binaryAvailable > 0
                            ? darkMode ? 'bg-gray-800 text-gray-500 hover:bg-emerald-900/30 hover:text-emerald-500' : 'bg-gray-200 text-gray-600 hover:bg-emerald-100 hover:text-emerald-600'
                            : darkMode ? 'bg-gray-800/50 text-gray-600' : 'bg-gray-100 text-gray-400'
                      }`}
                    >
                      {binaryAvailable > 0 ? binaryAvailable : '✔'}
                    </button>
                  )}
                  {mp.total > 0 && (
                    <button
                      onClick={() => isBinary ? selectTrack(selectedCategory, 'multiple') : null}
                      className={`text-xs px-1.5 py-0.5 rounded transition-colors ${
                        !isBinary
                          ? darkMode ? 'bg-violet-900/50 text-violet-400' : 'bg-violet-500 text-white'
                          : multipleAvailable > 0
                            ? darkMode ? 'bg-gray-800 text-gray-500 hover:bg-violet-900/30 hover:text-violet-500' : 'bg-gray-200 text-gray-600 hover:bg-violet-100 hover:text-violet-600'
                            : darkMode ? 'bg-gray-800/50 text-gray-600' : 'bg-gray-100 text-gray-400'
                      }`}
                    >
                      {multipleAvailable > 0 ? multipleAvailable : '✔'}
                    </button>
                  )}
                </div>
              );
          ...
          {/* Progress + nav row - ZONE 2: 32px */}
          <div className={`h-[32px] flex-shrink-0 flex items-center gap-2 px-3 ${darkMode ? 'bg-gray-900/50' : 'bg-white border-b border-gray-200'}`}>
            <button onClick={goBack} disabled={safeTrackIndex === 0} className={`text-xl font-bold ${safeTrackIndex === 0 ? (darkMode ? 'text-gray-700' : 'text-gray-300') : (darkMode ? 'text-gray-300 active:text-white' : 'text-gray-600 active:text-gray-900')}`}>◀</button>
            ...
            <button onClick={goForward} disabled={safeTrackIndex >= maxIndex} className={`text-xl font-bold ${safeTrackIndex >= maxIndex ? (darkMode ? 'text-gray-700' : 'text-gray-300') : (darkMode ? 'text-gray-300 active:text-white' : 'text-gray-600 active:text-gray-900')}`}>▶</button>
          </div>

        {/* Question + Evidence - ZONE 3: Question 60px + Evidence 32px collapsed */}
        <div className={`px-3 pt-2 flex-shrink-0 ${darkMode ? '' : 'bg-white'}`}>
          {/* Question text - fixed 60px (2 lines) with subtle glow */}
          <div
            className="min-h-[60px] h-[60px] flex items-start gap-2 p-2 -m-2 rounded-lg"
            style={{ background: darkMode ? 'linear-gradient(135deg, rgba(139,92,246,0.05) 0%, transparent 50%)' : 'transparent' }}
          >
            <h2 className={`text-base font-medium leading-normal flex-1 line-clamp-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{question.text}</h2>
            <button onClick={() => setShowQuestionFlagModal(question.id)} className={`text-sm flex-shrink-0 ${questionFlags[question.id] ? 'text-rose-400' : (darkMode ? 'text-gray-600 hover:text-gray-400' : 'text-gray-400 hover:text-gray-600')}`}>⚑</button>
          </div>
          
          {/* Evidence panel - 32px collapsed, scrollable when expanded */}
          <EvidencePanel
        ...
        {/* Answer Zone - ZONE 4: fills remaining space */}
        <div className={`flex-1 p-3 border-t overflow-y-auto ${darkMode ? 'border-gray-800 bg-gray-950' : 'border-gray-200 bg-gray-50'}`}>
          {/* Pending submit - undo window (tap anywhere to cancel) */}
          {pendingSubmit && pendingSubmit.questionId === question.id ? (
            (() => {
              const pendingColor = !isBinary ? MC_COLORS[pendingSubmit.answer % MC_COLORS.length] : null;
              return (
            <div 
                ...
                className={`py-4 px-4 ${
                  isBinary
                    ? pendingSubmit.answer === 0
                      ? darkMode ? 'bg-gradient-to-r from-emerald-900/40 to-emerald-800/20' : 'bg-gradient-to-r from-emerald-100 to-emerald-50'
                      : pendingSubmit.answer === 1
                        ? darkMode ? 'bg-gradient-to-r from-rose-900/40 to-rose-800/20' : 'bg-gradient-to-r from-rose-100 to-rose-50'
                        : darkMode ? 'bg-gray-800/50' : 'bg-gray-100'
                    : ''
                }`}
                style={!isBinary ? { background: darkMode ? `linear-gradient(to right, ${pendingColor.hex}30, ${pendingColor.hex}15)` : `linear-gradient(to right, ${pendingColor.hex}20, ${pendingColor.hex}08)` } : undefined}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span
                      ...
                      style={!isBinary ? { color: pendingColor.hex } : undefined}
                    >
                      {isBinary
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${
                        isBinary
                          ? pendingSubmit.answer === 0
                            ? darkMode ? 'bg-emerald-500/20 text-emerald-300' : 'bg-emerald-100 text-emerald-700 border border-emerald-300'
                            : pendingSubmit.answer === 1
                              ? darkMode ? 'bg-rose-500/20 text-rose-300' : 'bg-rose-100 text-rose-700 border border-rose-300'
                              : darkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-600 border border-gray-300'
                          : ''
                      }`}
                      style={!isBinary ? { background: `${pendingColor.hex}30`, color: pendingColor.hex } : undefined}
                    >
                      ◆{pendingSubmit.confidence}
                    </span>
                  </div>
                  <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>tap to undo</span>
                </div>
              </div>
              {/* Filling progress bar at bottom */}
              <div className={`absolute bottom-0 left-0 right-0 h-1 ${darkMode ? 'bg-gray-900' : 'bg-gray-300'}`}>
                <div 
                  className={`h-full ${
                    isBinary 
                  ...
                  style={{ 
                    animation: `fill ${undoDelay}s linear forwards`,
                    ...(isBinary ? {} : { background: pendingColor.hex })
                  }} 
                />
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            {(() => {
              const ansColor = !isBinary ? MC_COLORS[existingResponse.answer % MC_COLORS.length] : null;
              return (
            <div
              className={`py-2.5 px-3 rounded-xl ${
                isBinary
                  ? existingResponse.answer === 0
                    ? darkMode ? 'bg-emerald-900/20 border border-emerald-500/20' : 'bg-emerald-100 border border-emerald-300'
                    : existingResponse.answer === 1
                      ? darkMode ? 'bg-rose-900/20 border border-rose-500/20' : 'bg-rose-100 border border-rose-300'
                      : darkMode ? 'bg-gray-800/50 border border-gray-700' : 'bg-gray-200 border border-gray-300'
                  : ''
              }`}
              style={!isBinary ? {
                background: darkMode ? `${ansColor.hex}15` : `${ansColor.hex}10`,
                border: `1px solid ${darkMode ? ansColor.hex + '30' : ansColor.hex + '40'}`
              } : undefined}
            >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  <span
                    ...
                    style={!isBinary ? { color: ansColor.hex } : undefined}
                  >
                    {isBinary
                      ? (existingResponse.answer === 0 ? 'YES' : existingResponse.answer === 1 ? 'NO' : "Can't answer")
                      : allOptions[existingResponse.answer]}
                  </span>
                  <span className={`flex-shrink-0 ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>·</span>
                  <span className={`text-sm flex-shrink-0 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{CONFIDENCE_LABELS[existingResponse.confidence]}</span>
                  {/* Confidence dots */}
                  <div className="flex gap-0.5 flex-shrink-0">
                    {[1,2,3,4].map(i => (
                      <div
                        ...
                        style={!isBinary && i <= existingResponse.confidence ? { background: ansColor.hex } : undefined}
                      />
                    ))}
                  </div>
                </div>
                {/* Explanation button (amber) */}
                {answerExplanations[question.id] ? (
                  <button
                    onClick={() => setShowExplanationModal(question.id)}
                    className="flex-shrink-0 ml-2 w-6 h-6 rounded-full bg-amber-500 flex items-center justify-center"
                  ...
                  <button
                    onClick={() => setShowExplanationModal(question.id)}
                    className={`text-sm flex-shrink-0 ml-2 ${darkMode ? 'text-gray-600 hover:text-amber-400' : 'text-gray-400 hover:text-amber-500'}`}
              ...
              {existingResponse.cantAnswerReason && (
                <div className={`text-xs mt-1 ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>{existingResponse.cantAnswerReason}</div>
              )}
              {existingResponse.customAnswer && (
                <div className={`text-xs mt-1 italic ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>"{existingResponse.customAnswer}"</div>
              )}
              {existingResponse.explanation && (
                <div className={`text-xs mt-1 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>💬 {existingResponse.explanation}</div>
              )}
            </div>
              );
            })()}
            
            {/* Community results - always expanded */}
            {existingCommunity && existingCommunity.evaluators && (
              <div className={`p-2 rounded-lg ${darkMode ? 'bg-gray-900/50' : 'bg-gray-100'}`}>
                ...
                {isBinary ? (
                  <BinaryChart evaluators={existingCommunity.evaluators} userResponse={existingResponse} darkMode={darkMode} />
                ) : (
                  <MultipleChoiceChart evaluators={existingCommunity.evaluators} userResponse={existingResponse} options={options} darkMode={darkMode} />
                )}
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            {/* Undo banner - stays until resolved */}
            {justUndone && (
              <div className={`rounded-xl p-3 ${
                darkMode ? 'bg-amber-500/10 border border-amber-500/40' : 'bg-amber-100 border border-amber-300'
              }`}>
                <div className={`text-center text-xs font-bold mb-2 ${darkMode ? 'text-amber-400' : 'text-amber-700'}`}>
                  Undone
                </div>
                <button
                  onClick={() => { setTappedAnswer(null); setTapCount(0); setJustUndone(false); }}
            ...
            {/* First-time gesture hint */}
            {showGestureHint && !existingResponse && !justUndone && (
              <div
              ...
              >
                <div className={`text-xs ${darkMode ? 'text-violet-300' : 'text-violet-700'}`}>
                  <span className="font-medium">Tip:</span> Tap to select · Tap again to increase confidence
                </div>
                <span className={`text-xs ${darkMode ? 'text-violet-500' : 'text-violet-400'}`}>✕</span>
              </div>
            )}

            {/* Binary answers - Tap to select/increment, right-click to decrement */}
            {isBinary && (
                <>
                  <div className="grid grid-cols-2 gap-3">
                    {/* YES button */}
                    <button
                      onClick={() => !isSwiping && handleTap(0)}
                      onContextMenu={(e) => handleRightClick(e, 0)}
                      className={`relative py-5 rounded-xl font-bold text-lg transition-colors overflow-hidden select-none ${
                        tappedAnswer === 0
                        ...
                        background: tappedAnswer === 0
                          ? darkMode
                            ? `linear-gradient(135deg,
                                ${tapCount >= 4 ? '#10b981' : tapCount >= 3 ? '#059669' : tapCount >= 2 ? '#047857' : '#065f46'} 0%,
                                ${tapCount >= 4 ? '#6ee7b7' : tapCount >= 3 ? '#34d399' : tapCount >= 2 ? '#10b981' : '#059669'} 100%)`
                            : `linear-gradient(135deg,
                                ${tapCount >= 4 ? '#10b981' : tapCount >= 3 ? '#34d399' : tapCount >= 2 ? '#6ee7b7' : '#a7f3d0'} 0%,
                                ${tapCount >= 4 ? '#059669' : tapCount >= 3 ? '#10b981' : tapCount >= 2 ? '#34d399' : '#6ee7b7'} 100%)`
                          : undefined,
                        boxShadow: tappedAnswer === 0
                          ? `0 0 ${tapCount * 8}px rgba(16, 185, 129, ${0.2 + tapCount * 0.15})`
                          : 'none'
                      }}
                    >
                      <ConfidenceSegments level={tapCount} color="emerald" active={tappedAnswer === 0} darkMode={darkMode} />
                      {tappedAnswer === 0 && tapCount > 0 && !requireConfirmation && (
                    ...
                    {/* NO button */}
                    <button
                      onClick={() => !isSwiping && handleTap(1)}
                      onContextMenu={(e) => handleRightClick(e, 1)}
                      className={`relative py-5 rounded-xl font-bold text-lg transition-colors overflow-hidden select-none ${
                        tappedAnswer === 1
                          ? 'text-white ring-2 ring-rose-400'
                          : darkMode ? 'bg-gray-800 text-gray-300' : 'bg-white text-rose-600 border-2 border-rose-200 shadow-sm'
                      }`}
                      style={{
                        background: tappedAnswer === 1
                          ? darkMode
                            ? `linear-gradient(135deg,
                                ${tapCount >= 4 ? '#f43f5e' : tapCount >= 3 ? '#e11d48' : tapCount >= 2 ? '#be123c' : '#9f1239'} 0%,
                                ${tapCount >= 4 ? '#fda4af' : tapCount >= 3 ? '#fb7185' : tapCount >= 2 ? '#f43f5e' : '#e11d48'} 100%)`
                            : `linear-gradient(135deg,
                                ${tapCount >= 4 ? '#f43f5e' : tapCount >= 3 ? '#fb7185' : tapCount >= 2 ? '#fda4af' : '#fecdd3'} 0%,
                                ${tapCount >= 4 ? '#e11d48' : tapCount >= 3 ? '#f43f5e' : tapCount >= 2 ? '#fb7185' : '#fda4af'} 100%)`
                          : undefined,
                        boxShadow: tappedAnswer === 1
                          ? `0 0 ${tapCount * 8}px rgba(244, 63, 94, ${0.2 + tapCount * 0.15})`
                          : 'none'
                      }}
                    >
                      <ConfidenceSegments level={tapCount} color="rose" active={tappedAnswer === 1} darkMode={darkMode} />
                      {tappedAnswer === 1 && tapCount > 0 && !requireConfirmation && (
                  ...
                  {/* Confidence label */}
                  <div className={`text-center text-[10px] ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                    {tappedAnswer !== null
                      ? <span className={darkMode ? 'text-gray-400' : 'text-gray-600'}>
                          {CONFIDENCE_LABELS[tapCount]}
                          <span className={darkMode ? 'text-gray-600' : 'text-gray-400'}> · {tapCount < 4 ? 'tap +' : 'max'}</span>
                        </span>
                      : 'tap to answer'
                    }
                  </div>

                  {/* Submit row (confirmation mode OR undo flow) */}
                  {(requireConfirmation || justUndone) && tappedAnswer !== null && (
                    <div className="flex gap-2 mt-2">
                      <button
                        onClick={() => { submitWithConfidence(tappedAnswer, tapCount); setTappedAnswer(null); setTapCount(0); }}
                        className="flex-1 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-sm font-medium"
                      >
                        Submit
                      </button>
                      <button
                        onClick={() => { setTappedAnswer(null); setTapCount(0); }}
                        className={`px-3 py-2 rounded-lg text-sm ${darkMode ? 'bg-gray-800 text-gray-400 hover:bg-gray-700' : 'bg-gray-200 text-gray-600 hover:bg-gray-100'}`}
                      >
                        ↺
                      </button>
                    </div>
                ...
                <>
                  <div className="space-y-1">
                    {allOptions.map((opt, i) => {
                      ...
                      const conf = isSelected ? tapCount : 0;
                      ...
                      if (isNone) {
                        return (
                          <button
                            ...
                            className={`w-full py-2 px-3 rounded-lg text-left text-xs transition-all ${darkMode ? 'bg-gray-800/50 text-gray-500 hover:bg-gray-700/50' : 'bg-gray-200 text-gray-600 hover:bg-gray-100'}`}
                          >
                            None of the above
                          </button>
                        );
                      ...
                      return (
                        <button
                          key={i}
                          onClick={() => !isSwiping && handleTap(i)}
                          onContextMenu={(e) => handleRightClick(e, i)}
                          className={`w-full py-2.5 px-3 rounded-lg text-left text-sm transition-colors relative overflow-hidden select-none ${
                            isSelected
                              ? 'text-white ring-1'
                          ...
                          style={{
                            background: isSelected
                              ? `linear-gradient(135deg, ${optColor.hex}${Math.round((0.5 + conf * 0.1) * 255).toString(16).padStart(2, '0')} 0%, ${optColor.hex}${Math.round((0.7 + conf * 0.08) * 255).toString(16).padStart(2, '0')} 100%)`
                              : undefined,
                            ringColor: isSelected ? optColor.hex : undefined,
                            boxShadow: isSelected
                              ? `0 0 ${conf * 6}px ${optColor.hex}${Math.round((0.3 + conf * 0.15) * 255).toString(16).padStart(2, '0')}`
                              : 'none'
                          }}
                        >
                          <ConfidenceSegments level={conf} color={optColor.tailwind} active={isSelected} darkMode={darkMode} />
                          {isSelected && !requireConfirmation && (
                            <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background: `${optColor.hex}40` }}>
                              <div className="h-full" style={{ background: `${optColor.hex}aa`, animation: 'shrink 0.5s linear forwards' }} />
                            </div>
                          )}
                          <span className="relative z-10">
                            <span className={`mr-1.5 text-xs ${isSelected ? 'text-white/70' : (darkMode ? 'text-gray-400/70' : 'text-gray-400')}`}>{String.fromCharCode(65 + i)}</span>
                            {opt}
                          </span>
                          {isSelected && (
                  ...
                  {/* Submit row (confirmation mode OR undo flow) for MC */}
                  {(requireConfirmation || justUndone) && tappedAnswer !== null && (
                    <div className="flex gap-2 mt-2">
                      <button
                        onClick={() => { submitWithConfidence(tappedAnswer, tapCount); setTappedAnswer(null); setTapCount(0); }}
              ...
              {/* Inline explanation - single row chips + input */}
              {showExplanationInput && (
                <div className={`mt-2 flex items-center gap-1.5 p-1.5 rounded-lg ${darkMode ? 'bg-gray-800/30' : 'bg-gray-100'}`}>
                  {['Confident', 'Unsure', 'Guess'].map(r => (
                    <button key={r} onClick={() => setPendingExplanation(prev => prev === r ? '' : r)}
                      className={`px-2 py-1 rounded text-[10px] whitespace-nowrap ${pendingExplanation === r ? 'bg-violet-500 text-white' : darkMode ? 'bg-gray-700 text-gray-400' : 'bg-white text-gray-600 border border-gray-200'}`}
                    >{r}</button>
                  ))}
                  <input
                    type="text"
                    ...
                    placeholder="Note..."
                    className={`flex-1 min-w-0 px-2 py-1 rounded text-[10px] border-0 focus:outline-none ${
                      darkMode ? 'bg-gray-900/50 text-white placeholder-gray-600' : 'bg-white text-gray-900 placeholder-gray-400'
                    }`}
                  />
                </div>
              )}

              {/* Action buttons row - 4 col grid (or 2 col during undo) */}
              {pendingSubmit && pendingSubmit.questionId === question.id ? (
                <div className="grid grid-cols-2 gap-2 mt-2">
                  <button
                    onClick={() => { cancelSubmit(); skipQuestion(); }}
                    className={`py-2 px-1 rounded-lg text-[11px] transition-colors ${darkMode ? 'bg-gray-800/50 text-gray-500 hover:bg-gray-700/50' : 'bg-gray-200 text-gray-600 hover:bg-gray-100'}`}
                  >
                    Skip
                  </button>
                  <button
                ...
                <div className="grid grid-cols-4 gap-2 mt-2">
                  <button
                    onClick={() => isBinary ? setShowCantAnswerModal(true) : setShowNoneOptionModal(true)}
                    className={`py-2 px-1 rounded-lg text-[11px] transition-colors ${darkMode ? 'bg-gray-800/50 text-gray-500 hover:bg-gray-700/50' : 'bg-gray-200 text-gray-600 hover:bg-gray-100'}`}
                  >
                    Can't
                  </button>
                  <button
                    ...
                    className={`py-2 px-1 rounded-lg text-[11px] transition-colors ${
                      showExplanationInput
                        ? 'bg-violet-900/30 text-violet-400'
                        : darkMode ? 'bg-gray-800/50 text-gray-500 hover:bg-gray-700/50' : 'bg-gray-200 text-gray-600 hover:bg-gray-100'
                    }`}
                  >
                    Explain
                  </button>
                  ...
                  >
                    {savedQuestions.has(question?.id) ? '✓ Saved' : 'Save'}
                  </button>
                </div>
              )}
  ...
  if (screen === 'history') {
    ...
    return (
      ...
      >
        <div className={`flex-1 flex flex-col ${darkMode ? 'text-white' : 'text-gray-900'}`}>
          <div className={`h-[44px] flex-shrink-0 flex items-center gap-2 px-3 border-b ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
            <button onClick={() => setScreen('categories')} className={darkMode ? 'text-gray-400' : 'text-gray-500'}>←</button>
            <span className="font-medium flex-1">History</span>
            <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>{answeredQuestions.length} answered</span>
          </div>
                ...
                <div key={q.id} className={`rounded-lg p-3 ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className={`text-xs font-medium ${
                          q.type === 'binary'
                            ? (q.response.answer === 0 ? (darkMode ? 'text-emerald-400' : 'text-emerald-600') : q.response.answer === 1 ? (darkMode ? 'text-rose-400' : 'text-rose-600') : 'text-gray-400')
                            : (darkMode ? 'text-violet-400' : 'text-violet-600')
                        }`}>
                          {getAnswerLabel(q)}
                        </span>
                        <ConfidenceDots level={q.response.confidence} />
                        <span className="text-[10px] text-gray-500">{CONFIDENCE_LABELS[q.response.confidence]}</span>
                        {/* Flag/explanation indicators */}
                        {questionFlags[q.id] && (
                          <span className="text-[10px] text-rose-400" title={questionFlags[q.id]}>⚑</span>
                        )}
                        {answerExplanations[q.id] && (
                          <span className="text-[10px] text-amber-400" title={answerExplanations[q.id]}>✎</span>
                        )}
                      </div>
                      {/* Show explanation if exists */}
                      {q.response.explanation && (
                        <div className={`text-[10px] mt-1 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>💬 {q.response.explanation}</div>
                      )}
                    </div>
                  </div>
                  ...
                  {q.community && (
                    <button onClick={() => setEvidenceExpanded(isExpanded ? null : `history-${q.id}`)}
                      ...
                      <div className={`flex items-center justify-between text-[10px] ${darkMode ? 'text-gray-600' : 'text-gray-500'}`}>
                      ...
                      {isExpanded && (
                        <div className="mt-2">
                          {q.type === 'binary' ? (
                            <BinaryChart evaluators={q.community.evaluators} userResponse={q.response} darkMode={darkMode} />
                          ) : (
                            <MultipleChoiceChart evaluators={q.community.evaluators} userResponse={q.response} options={q.options} darkMode={darkMode} />
                          )}
                        </div>
                      )}
                    </button>
  ...
  if (screen === 'profile') {
    ...
    // Category breakdown with confidence distribution
    const categoryBreakdown = {};
    answeredList.forEach(r => {
      const q = shuffledQuestions.find(q => q.id === r.id);
      if (q) {
        if (!categoryBreakdown[q.category]) {
          categoryBreakdown[q.category] = { count: 0, confDist: [0, 0, 0, 0] };
        }
    ...
    return (
      <PhoneFrame 
        ...
        onUserClick={goToProfile}
      >
        <div className={`flex-1 flex flex-col overflow-hidden ${darkMode ? 'text-white' : 'text-gray-900'}`}>
          <div className={`h-[44px] flex-shrink-0 flex items-center gap-2 px-3 border-b ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
            <button onClick={() => setScreen('categories')} className={darkMode ? 'text-gray-400' : 'text-gray-500'}>←</button>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            {/* User info */}
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-gradient-to-br from-violet-500 to-fuchsia-500 rounded-full flex items-center justify-center font-bold text-lg text-white">{username[0]?.toUpperCase()}</div>
              <div>
                <h2 className="font-medium">{username}</h2>
                {userTeams.length > 0 && (
                  <div className="flex gap-1 mt-1">
                    {userTeams.map(t => <span key={t} className={`text-xs px-1.5 py-0.5 rounded ${darkMode ? 'bg-violet-900/50 text-violet-300' : 'bg-violet-100 text-violet-700'}`}>{t}</span>)}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className={`rounded-lg p-3 text-center ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
                <div className={`text-2xl font-bold ${darkMode ? 'text-emerald-400' : 'text-emerald-500'}`}>{turkEarnings}</div>
            ...
            {answered > 0 && (
              <div className={`rounded-lg p-3 ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
                <div className={`text-xs mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Confidence Distribution</div>
            ...
            {/* Category breakdown with confidence shading */}
            {Object.keys(categoryBreakdown).length > 0 && (
              <div className={`rounded-lg p-3 ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
                <div className={`text-xs mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>By Category</div>
                <div className="space-y-2">
                          ...
                          {catConf.map((c, i) => {
                            ...
                            return (
                              <div
                                ...
                                style={{ width: `${segPct}%`, background: colors[i] }}
                              />
                            );
                          })}
                        </div>
            ...
            <div className={`rounded-lg p-3 ${darkMode ? 'bg-gray-900/50 border border-gray-800' : 'bg-white border border-gray-300'}`}>
              <div className={`text-xs mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Coming Soon</div>
              <div className="space-y-1.5 text-xs text-gray-500">
                <div className="flex items-center gap-2">
                  <span className="text-amber-400">◆</span>
                  <span>Accuracy tracking & calibration</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-amber-400">◆</span>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-amber-400">◆</span>
                  <span>Prediction resolution history</span>
                </div>
              </div>
            </div>
          </div>
      ...
      >
        <div className={`flex-1 flex flex-col ${darkMode ? 'text-white' : 'text-gray-900'}`}>
          ...
          <div className="flex-1 p-4 space-y-3 overflow-y-auto">
            ...
            {/* Stats bar */}
            <div className={`flex gap-3 p-3 rounded-lg ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
              <div className="flex-1 text-center">
                <div className={`text-lg font-bold ${darkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>{turkEarnings}</div>
              ...
              <div className="flex-1 text-center">
                <div className={`text-lg font-bold ${darkMode ? 'text-amber-400' : 'text-amber-600'}`}>{totalAvailable}</div>
            ...
            {TURK_TASK_TYPES.map(type => {
              ...
              const colorMap = darkMode ? colorMapDark : colorMapLight;
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-lg text-left transition-all ${
                    count > 0
                      ? `${darkMode ? 'bg-gray-900 hover:bg-gray-800' : 'bg-white border border-gray-200 hover:bg-gray-50 shadow-sm'}`
                      : 'opacity-40'
                  }`}
                >
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm flex-1 font-medium ${darkMode ? 'text-amber-300' : 'text-amber-700'}`}>
                        ...
                        {availableTypes.slice(0, 3).map(type => {
                          ...
                          return (
                            <button 
                              key={type.id}
                              onClick={() => { setSelectedTurkType(type.id); setScreen('turk-type'); }}
                              className={`w-full p-3 rounded-lg flex items-center gap-3 ${darkMode ? 'bg-gray-900 hover:bg-gray-800' : 'bg-white border border-gray-200 hover:bg-gray-50'}`}
                            >
                              <span className="text-xl">{type.icon}</span>
                              <span className={`flex-1 text-left text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{type.name}</span>
                  ...
                  <div>
                    ...
                    {activeTasks.map(task => {
                      ...
                      return (
                        <div 
                          key={task.id}
                          className={`mb-2 p-3 rounded-lg ${darkMode ? 'bg-amber-500/10 border border-amber-500/20' : 'bg-amber-100 border border-amber-300'}`}
                        >
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className={`px-3 py-2 text-xs rounded-lg ${darkMode ? 'bg-gray-700 text-gray-300' : 'bg-white text-gray-700 border border-gray-200'}`}
                            >
                              Later
                            </button>
                            <button
                              onClick={() => handleDeclineTask(task.id)}
                              className={`px-3 py-2 text-xs rounded-lg ${darkMode ? 'bg-gray-800 text-gray-500' : 'bg-gray-100 text-gray-500'}`}
                            >
                              Decline
                            </button>
                          </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-2 mb-2">
                            ...
                            <div className="flex-1">
                              <div className={`font-medium text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{task.title}</div>
                              <div className={`text-xs ${darkMode ? 'text-gray-600' : 'text-gray-500'}`}>{typeInfo?.name}</div>
                            </div>
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className={`flex-1 py-2 text-xs font-medium rounded-lg ${darkMode ? 'bg-gray-700 text-white' : 'bg-white text-gray-800 border border-gray-200'}`}
                            >
                              Do Now
                            </button>
                            <button
                              onClick={() => handleUnsaveTask(task.id)}
                              className={`px-3 py-2 text-xs rounded-lg ${darkMode ? 'bg-gray-800 text-gray-400' : 'bg-white text-gray-600 border border-gray-200'}`}
                            >
                              Restore
                            </button>
                            <button
  ...
  if (screen === 'turk-type') {
    ...
    const getAccessBadge = (task) => {
      if (task.access === 'open') return { text: 'Open', color: darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-600 border border-gray-300' };
      if (task.access === 'rated') return { text: `Rating ${task.minRating}+`, color: darkMode ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-700 border border-blue-300' };
      if (task.access === 'team') return { text: 'Team', color: darkMode ? 'bg-violet-500/20 text-violet-400' : 'bg-violet-100 text-violet-700 border border-violet-300' };
      if (task.access === 'assigned') return { text: 'Assigned', color: darkMode ? 'bg-amber-500/20 text-amber-400' : 'bg-amber-100 text-amber-700 border border-amber-300' };
      return { text: '', color: '' };
    };
          ...
          <div className="flex-1 p-3 space-y-1.5 overflow-y-auto">
            {tasks.length === 0 ? (
              ...
              tasks.map((task, index) => {
                ...
                const showBadge = task.access !== 'open'; // Only show badge for non-open tasks
                return (
                  <button
                    key={task.id}
                    onClick={() => {
                    ...
                    className={`w-full p-2 rounded-lg text-left flex items-center gap-2 ${darkMode ? 'bg-gray-900 hover:bg-gray-800' : 'bg-white border border-gray-200 hover:bg-gray-50'}`}
                  >
                    {/* Task number */}
                    <span className={`text-xs font-medium w-5 h-5 rounded flex items-center justify-center flex-shrink-0 ${darkMode ? 'bg-gray-800 text-gray-500' : 'bg-gray-200 text-gray-600'}`}>
                      {index + 1}
                    </span>
                    {/* Task info */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {showBadge && (
                          <span className={`text-[10px] px-1 py-0.5 rounded flex-shrink-0 ${badge.color}`}>{badge.text}</span>
                        )}
                      </div>
                    ...
                    <div className="text-right flex-shrink-0">
                      <div className={`font-bold text-xs ${darkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>+{task.reward}</div>
                      <div className="text-[10px] text-gray-500">{task.timeLimit}s</div>
                    </div>
                  </button>
                );
              })
        ...
        <div className={`flex-1 flex flex-col ${darkMode ? 'text-white' : 'text-gray-900'}`}>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 p-4 overflow-y-auto">
              ...
              {task.media && (
                <div className="mb-4">
                  ...
                  {task.media.type === 'video' && (
                    <div className={`relative rounded-lg overflow-hidden h-36 ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                  ...
                  {task.media.type === 'document' && (
                    <div className={`rounded-lg h-32 flex items-center justify-center ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                      <div className="text-center">
                        <div className="text-4xl mb-2">📄</div>
                        <div className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Document viewer</div>
                      </div>
                    </div>
                  )}
                  {task.media.type === 'comparison' && task.media.images && (
                    <div className="grid grid-cols-2 gap-2">
                      {task.media.images.map((img, i) => (
                        <div key={i} className={`rounded-lg overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                          <img src={img} alt={task.media.labels?.[i] || `Profile ${i+1}`} className="w-full h-28 object-cover" />
                          <div className={`text-xs text-center py-1 ${darkMode ? 'text-gray-400 bg-gray-900' : 'text-gray-600 bg-gray-100'}`}>
                  ...
                  {task.media.type === 'audio' && (
                    <div className={`rounded-lg p-4 flex items-center gap-3 ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                      <button className="w-10 h-10 bg-violet-600 rounded-full flex items-center justify-center text-white">▶</button>
                      <div className={`flex-1 h-2 rounded-full ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                        <div className="w-1/3 h-full bg-violet-500 rounded-full" />
                      </div>
                      <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>0:00</span>
                    </div>
                  )}
                </div>
              )}
                ...
                <div className={`rounded-lg mb-4 ${darkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
                  <div className={`p-3 border-b ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>{task.playerData.accountAge} old</span>
                    </div>
                    {task.playerData.avgResponseTime && (
                      <div className={`text-xs mt-1 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Avg response: {task.playerData.avgResponseTime}</div>
                    )}
                  </div>
                  {task.playerData.answers && (
                    <div className="p-2 max-h-40 overflow-y-auto">
                      <div className={`text-xs mb-2 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Recent answers:</div>
                      {task.playerData.answers.slice(0, 6).map((ans, i) => (
                        <div key={i} className={`flex items-center justify-between py-1 text-xs border-b last:border-0 ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
                          <span className={`truncate flex-1 mr-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{ans.q}</span>
                          <span className={`font-medium ${ans.a === 'YES' ? 'text-emerald-400' : ans.a === 'NO' ? 'text-rose-400' : 'text-violet-400'}`}>
                            {ans.a}
                          </span>
                          <span className={`ml-2 ${darkMode ? 'text-gray-600' : 'text-gray-500'}`}>◆{ans.conf}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  {task.playerData.calibration && (
                    <div className="p-2">
                      <div className={`text-xs mb-2 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Calibration ({task.playerData.totalAnswers} total):</div>
                      {task.playerData.calibration.map((cal, i) => (
                        <div key={i} className="flex items-center gap-2 py-1 text-xs">
                          <span className={`w-16 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Conf {cal.conf}:</span>
                          <div className={`flex-1 h-2 rounded-full overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                          ...
                          <span className={`w-10 text-right ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{cal.accuracy}</span>
                          <span className={`w-8 ${darkMode ? 'text-gray-600' : 'text-gray-500'}`}>({cal.count})</span>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {task.previousVotes && (
                <div className={`p-3 rounded-lg mb-4 ${darkMode ? 'bg-amber-500/10 border border-amber-500/30' : 'bg-amber-100 border border-amber-300'}`}>
              ...
              {timeExpired ? (
                <div className="text-center py-4">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm ${darkMode ? 'bg-gray-700 text-white' : 'bg-white text-gray-800 border border-gray-200'}`}
                  >
                    Back to Tasks
                  </button>
                </div>
  ...
  if (screen === 'qotd') {
      ...
      return (
        <div className="mb-3">
          <div className="flex items-center justify-between mb-1">
            <span className={`text-sm ${isWinner ? 'font-bold' : ''} ${color === 'emerald' ? (darkMode ? 'text-emerald-400' : 'text-emerald-600') : (darkMode ? 'text-rose-400' : 'text-rose-600')} ${isUserSide ? 'underline' : ''}`}>
              {label}{isUserSide && ' •'}
            </span>
            <span className={`text-sm ${isWinner ? 'font-bold' : ''} ${color === 'emerald' ? (darkMode ? 'text-emerald-400/70' : 'text-emerald-600') : (darkMode ? 'text-rose-400/70' : 'text-rose-600')}`}>{pct}%</span>
          </div>
          <div className={`h-6 rounded-lg overflow-hidden flex ${darkMode ? 'bg-gray-800/50' : 'bg-gray-200'} relative ${isUserSide ? 'ring-1 ring-offset-1' : ''}`} style={{ ringColor: color === 'emerald' ? '#34d399' : '#fb7185', ringOffsetColor: darkMode ? '#030712' : '#f9fafb' }}>
            {/* High confidence (4) on left */}
            {[3, 2, 1, 0].map(i => {
    ...
    const MCOptionBar = ({ dist, label, pct, isUser, isWinner, color }) => {
      ...
      return (
        <div className="mb-2">
          <div className="flex items-center justify-between mb-1">
            <span className={`text-xs ${isWinner ? (darkMode ? 'font-bold text-white' : 'font-bold text-gray-900') : (darkMode ? 'text-gray-400' : 'text-gray-600')} ${isUser ? 'underline' : ''}`}>
              {label}{isUser && ' •'}
            </span>
            <span className={`text-xs ${isWinner ? (darkMode ? 'font-bold text-white' : 'font-bold text-gray-900') : (darkMode ? 'text-gray-500' : 'text-gray-500')}`}>{pct}%</span>
          </div>
          <div className={`h-5 rounded overflow-hidden flex ${darkMode ? 'bg-gray-800/50' : 'bg-gray-200'} ${isUser ? 'ring-1 ring-violet-400' : ''}`}>
            {/* High confidence (4) on left */}
            {[3, 2, 1, 0].map(i => {
              const count = dist[i];
    ...
    const letters = ['A', 'B', 'C', 'D', 'E'];
    const mcColors = MC_COLORS.map(c => c.hex);
      ...
      >
        {hasAnswered ? (
          ...
          >
            ...
            {/* Question area - matches regular flow */}
            <div className={`flex-shrink-0 p-3 space-y-2 ${darkMode ? 'bg-gray-950' : 'bg-white'}`}>
              {/* Question text with subtle glow */}
              <div
                className="min-h-[60px] h-[60px] flex items-start gap-2 p-2 -m-2 rounded-lg"
                style={{ background: darkMode ? 'linear-gradient(135deg, rgba(139,92,246,0.05) 0%, transparent 50%)' : 'transparent' }}
              >
                <h2 className={`text-base font-medium leading-normal flex-1 line-clamp-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{qotd.text}</h2>
                <button
                  onClick={(e) => { e.stopPropagation(); setShowQuestionFlagModal(qotd.id); }}
                  className={`text-sm flex-shrink-0 ${questionFlags[qotd.id] ? 'text-rose-400' : (darkMode ? 'text-gray-600 hover:text-gray-400' : 'text-gray-400 hover:text-gray-600')}`}
                >⚑</button>
              </div>
              ...
              <EvidencePanel
                ...
                teamEvidence={null}
                userTeams={userTeams}
                expanded={qotdEvidenceExpanded}
                onToggle={(e) => { if (e) e.stopPropagation(); setQotdEvidenceExpanded(!qotdEvidenceExpanded); }}
                showPost={false}
                darkMode={darkMode}
              />
            </div>
            ...
            <div className="flex-1 p-4 flex flex-col overflow-y-auto">
              {/* Results */}
              <div className={`rounded-xl p-4 mb-4 ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
                <div className="flex items-center justify-between mb-3">
                  <span className={`text-xs uppercase tracking-wide ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Results</span>
                  <span className={`text-xs ${darkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>✔ Recorded</span>
                </div>
              ...
              {/* Streak/stats */}
              <div className={`rounded-lg p-3 ${darkMode ? 'bg-emerald-900/20 border border-emerald-500/20' : 'bg-emerald-100 border border-emerald-300'}`}>
                <div className="flex items-center justify-between text-sm">
                  <span className={darkMode ? 'text-gray-400' : 'text-gray-600'}>Daily streak</span>
                  <span className={`font-bold ${darkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>🔥 {QOTD_DATA.stats.streak + 1} days</span>
                </div>
              </div>

              {/* Explanation button */}
              <div className={`mt-3 p-2 rounded-lg ${darkMode ? 'bg-gray-800/50' : 'bg-white/80 border border-gray-200'}`}>
                {answerExplanations[qotd.id] ? (
                  <button
                    onClick={(e) => { e.stopPropagation(); setShowExplanationModal(qotd.id); }}
                  ...
                  >
                    ...
                    <span className={darkMode ? 'text-amber-400' : 'text-amber-700'}>{answerExplanations[qotd.id]}</span>
                  </button>
                ) : (
                  <button
                    onClick={(e) => { e.stopPropagation(); setShowExplanationModal(qotd.id); }}
                    className={`w-full text-left text-xs ${darkMode ? 'text-gray-400 hover:text-gray-300' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    ✎ Add note to answer
                  </button>
                )}
          ...
          <div className={`flex-1 flex flex-col ${darkMode ? 'text-white' : 'text-gray-900'} relative`}>
            ...
            <div className={`flex-shrink-0 p-3 space-y-2 ${darkMode ? 'bg-gray-950' : 'bg-white'}`}>
              ...
              >
                <h2 className={`text-base font-medium leading-normal flex-1 line-clamp-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{qotd.text}</h2>
                <button
                  onClick={() => setShowQuestionFlagModal(qotd.id)}
                  className={`text-sm flex-shrink-0 ${questionFlags[qotd.id] ? 'text-rose-400' : (darkMode ? 'text-gray-600 hover:text-gray-400' : 'text-gray-400 hover:text-gray-600')}`}
                >⚑</button>
              </div>
              ...
              <EvidencePanel
                ...
                userTeams={userTeams}
                expanded={qotdEvidenceExpanded}
                onToggle={() => setQotdEvidenceExpanded(!qotdEvidenceExpanded)}
                showPost={false}
                darkMode={darkMode}
              />
            </div>
            ...
            <div className={`p-3 border-t ${darkMode ? 'border-gray-800 bg-gray-950' : 'border-gray-200 bg-gray-50'}`}>
              {qotdPendingSubmit ? (
                ...
                /* Binary Yes/No buttons - mirrors regular question flow */
                <>
                  <div className="grid grid-cols-2 gap-3">
                    {/* YES button */}
                    <button
                      onClick={() => handleQotdTap(0)}
                      ...
                      style={{
                        background: qotdTappedAnswer === 0
                          ? darkMode
                            ? `linear-gradient(135deg,
                                ${qotdTapCount >= 4 ? '#10b981' : qotdTapCount >= 3 ? '#059669' : qotdTapCount >= 2 ? '#047857' : '#065f46'} 0%,
                                ${qotdTapCount >= 4 ? '#6ee7b7' : qotdTapCount >= 3 ? '#34d399' : qotdTapCount >= 2 ? '#10b981' : '#059669'} 100%)`
                            : `linear-gradient(135deg,
                                ${qotdTapCount >= 4 ? '#10b981' : qotdTapCount >= 3 ? '#34d399' : qotdTapCount >= 2 ? '#6ee7b7' : '#a7f3d0'} 0%,
                                ${qotdTapCount >= 4 ? '#059669' : qotdTapCount >= 3 ? '#10b981' : qotdTapCount >= 2 ? '#34d399' : '#6ee7b7'} 100%)`
                          : undefined,
                        transform: qotdTappedAnswer === 0 ? `scale(${1 + qotdTapCount * 0.015})` : 'scale(1)',
                      }}
                    >
                      <ConfidenceSegments level={qotdTapCount} color="emerald" active={qotdTappedAnswer === 0} darkMode={darkMode} />
                      <span className="relative z-10">YES</span>
                      {qotdTappedAnswer === 0 && (
                        <span className="absolute top-2 right-2 text-sm font-bold bg-white/30 w-7 h-7 rounded-full flex items-center justify-center">
                          {qotdTapCount}
                    ...
                    <button
                      ...
                      style={{
                        background: qotdTappedAnswer === 1
                          ? darkMode
                            ? `linear-gradient(135deg,
                                ${qotdTapCount >= 4 ? '#f43f5e' : qotdTapCount >= 3 ? '#e11d48' : qotdTapCount >= 2 ? '#be123c' : '#9f1239'} 0%,
                                ${qotdTapCount >= 4 ? '#fda4af' : qotdTapCount >= 3 ? '#fb7185' : qotdTapCount >= 2 ? '#f43f5e' : '#e11d48'} 100%)`
                            : `linear-gradient(135deg,
                                ${qotdTapCount >= 4 ? '#f43f5e' : qotdTapCount >= 3 ? '#fb7185' : qotdTapCount >= 2 ? '#fda4af' : '#fecdd3'} 0%,
                                ${qotdTapCount >= 4 ? '#e11d48' : qotdTapCount >= 3 ? '#f43f5e' : qotdTapCount >= 2 ? '#fb7185' : '#fda4af'} 100%)`
                          : undefined,
                        transform: qotdTappedAnswer === 1 ? `scale(${1 + qotdTapCount * 0.015})` : 'scale(1)',
                      }}
                    >
                      <ConfidenceSegments level={qotdTapCount} color="rose" active={qotdTappedAnswer === 1} darkMode={darkMode} />
                      <span className="relative z-10">NO</span>
                      {qotdTappedAnswer === 1 && (
                        <span className="absolute top-2 right-2 text-sm font-bold bg-white/30 w-7 h-7 rounded-full flex items-center justify-center">
                          {qotdTapCount}
                  ...
                  <div className={`text-center text-[10px] ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                    {qotdTappedAnswer !== null
                      ? <span className={darkMode ? 'text-gray-300' : 'text-gray-600'}>
                          {CONFIDENCE_LABELS[qotdTapCount]}
                          <span className={darkMode ? 'text-gray-600' : 'text-gray-400'}> · {qotdTapCount < 4 ? 'tap +' : 'max'}</span>
                        </span>
                      : 'tap to answer'
                    }
                  </div>
                  ...
                  {qotdTappedAnswer !== null && (
                    <div className="flex gap-2 mt-2">
                      <button
                        onClick={() => { submitQotdWithConfidence(qotdTappedAnswer, qotdTapCount); setQotdTappedAnswer(null); setQotdTapCount(0); }}
                        className="flex-1 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-sm font-medium"
                      >
                        Submit
                      </button>
                      <button
                        onClick={() => { setQotdTappedAnswer(null); setQotdTapCount(0); }}
                        className={`px-3 py-2 rounded-lg text-sm ${darkMode ? 'bg-gray-800 text-gray-400 hover:bg-gray-700' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-100'}`}
                      >
                        ↺
                      </button>
                    </div>
                ...
                /* MC option buttons - mirrors regular question flow */
                <>
                  <div className="space-y-1">
                    {options.map((opt, i) => {
                      const isSelected = qotdTappedAnswer === i;
                      const conf = isSelected ? qotdTapCount : 0;
                      const optColor = MC_COLORS[i % MC_COLORS.length];

                      return (
                        <button
                          key={i}
                          onClick={() => handleQotdTap(i)}
                          className={`w-full py-2.5 px-3 rounded-lg text-left text-sm transition-all relative overflow-hidden select-none ${
                            isSelected
                              ? 'text-white ring-1'
                              : darkMode ? 'bg-gray-800/50 text-gray-300 hover:bg-gray-800' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50'
                          }`}
                          style={{
                            background: isSelected ? `linear-gradient(135deg, ${optColor.shades[Math.min(conf, 3)]} 0%, ${optColor.shades[Math.max(0, Math.min(conf, 3) - 1)]} 100%)` : undefined,
                            ringColor: isSelected ? optColor.hex : undefined,
                            transform: isSelected ? `scale(${1 + conf * 0.01})` : 'scale(1)',
                          }}
                        >
                          <ConfidenceSegments level={conf} color={optColor.tailwind} active={isSelected} darkMode={darkMode} />
                          <span className="font-medium mr-2" style={isSelected ? { color: 'white' } : { color: optColor.hex }}>{letters[i]}.</span>
          ...
          <div className="flex-1 p-4 space-y-4 overflow-y-auto">
            ...
            <div className={`rounded-lg p-4 ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
              <div className="flex items-center justify-between">
                ...
                <button 
                  onClick={() => setDarkMode(!darkMode)}
                  className={`relative w-14 h-8 rounded-full transition-colors ${darkMode ? 'bg-violet-600' : 'bg-gray-100'}`}
                >
            ...
            <div className={`rounded-lg p-4 ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
              ...
              <input
                ...
                className={`w-full h-2 rounded-lg appearance-none cursor-pointer accent-violet-500 ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}
              />
            ...
            {/* Quick Submit (no confirmation button) */}
            <div className={`rounded-lg p-4 ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium">Quick Submit</div>
                  <div className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>
                    {requireConfirmation ? 'Tap then press Submit' : 'Auto-submit after tap'}
                  </div>
                </div>
                <button
                  onClick={() => setRequireConfirmation(!requireConfirmation)}
                  className={`relative w-14 h-8 rounded-full transition-colors ${!requireConfirmation ? 'bg-violet-600' : (darkMode ? 'bg-gray-600' : 'bg-gray-300')}`}
                >
                  <div className={`absolute top-1 w-6 h-6 bg-white rounded-full shadow transition-transform ${!requireConfirmation ? 'translate-x-7' : 'translate-x-1'}`} />
                </button>
              </div>
            </div>
            ...
            <div className={`rounded-lg p-4 ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative w-14 h-8 rounded-full transition-colors ${showStreak ? 'bg-violet-600' : (darkMode ? 'bg-gray-600' : 'bg-gray-300')}`}
                >
            ...
            <div className={`rounded-lg p-4 ${darkMode ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative w-14 h-8 rounded-full transition-colors ${tipsOn ? 'bg-violet-600' : (darkMode ? 'bg-gray-600' : 'bg-gray-300')}`}
                >
            ...
            <div className={`text-xs ${darkMode ? 'text-gray-600' : 'text-gray-500'} text-center mt-4`}>
              <p>Aura Player vI</p>
              <p className="mt-1">Swipe ← → to navigate · Tap to answer</p>
            </div>
          </div>
        </div>
      </PhoneFrame>
