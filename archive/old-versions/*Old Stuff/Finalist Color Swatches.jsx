...
export default function ColorComparisonGrid() {
  const versions = [
    ...
    { name: 'V9', colors: ['blue', 'violet', 'teal', 'cyan', 'pink'] },
  ];

  const shades = {
    violet: ['#6d28d9', '#8b5cf6', '#a78bfa', '#c4b5fd'],
    purple: ['#7e22ce', '#a855f7', '#c084fc', '#d8b4fe'],
          ...
          <div key={v.name} style={{ background: '#0f172a', borderRadius: 16, padding: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {v.colors.map((color, i) => (
                <div key={i} style={{ display: 'flex', height: 32, borderRadius: 16, overflow: 'hidden', position: 'relative' }}>
                  ...
                  <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#fff', fontSize: 12, fontWeight: 600 }}>{color}</span>
                </div>
              ))}
            </div>
          </div>
